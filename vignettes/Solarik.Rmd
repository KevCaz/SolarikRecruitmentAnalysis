...
2- simulated annealing is performed (`GenSA()` from [GenSA](https://cran.r-project.org/web/packages/GenSA/index.html) package) to minimize
...
The line below (not evaluated) allows to run for the 16<sup>th</sup> model.

```{r eval = FALSE}
launchIt(iter = 16, simu = simuDesign, mxt = 100,
  record = "inst/res/test.txt", quiet = TRUE, path = "inst/dataReady/")
```

In addition to the parameters estimation, we record estimation performed by
`GenSA` in order to compute a confident interval.
